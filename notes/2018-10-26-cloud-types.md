...
## Key concepts from this paper
  ...
  - the paper is specifically about *programmability* of eventually consistent data structures, and providing a programming model where nothing about the fact that we're dealing with replicated data leaks through to the programmer, *except* for calls to `yield` and `flush` here and there.  So, it's important for the programmer to understand the semantics of `yield` and `flush`, but it's not important *for the programmer* to understand the stuff about revision diagrams and join conditions and so on.
